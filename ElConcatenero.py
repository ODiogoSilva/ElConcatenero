...
def dataset_creator(infile_list):
	...
	storage,part_list,sizes = ElParsito.Elparsito(arg.InputFormat,initial_storage,infile_list,arg.OutputFormat,tab_delimited_loci)
	return storage,part_list,sizes,taxa_order
	
def output_creator(output_format,storage,part_list,sizes,taxa_order,outfile):
	if output_format == "ima2":
		out_file = open(outfile+".txt","w")
		info = input("Please provide (separated by semicolon): number of populations, name of populations, population tree:\nExample: 3;pop1 pop2 pop3; ((0,1):2\n>>> ")
		...
		for file_i in arg.infile:
			storage,part_list,sizes,taxa_order = dataset_creator(file_i.split())
...
if arg.OutputFormat == "ima2":
	output_creator (arg.OutputFormat)
elif arg.Conversion:
	for file_i in arg.infile:
		storage,part_list,sizes,taxa_order = dataset_creator(file_i.split())
		output_creator(arg.OutputFormat,storage,part_list,sizes,taxa_order,file_i.split(".")[0])
else:
	if arg.outfile == None:
